...
export async function processBot(body) {
  ...
  const startText =
    `Hey there! My name is KweeksBot. ` +
  ...
  const helpNoneText = `No help message affiliated with ${value}.`;
  const helpNonPrivateText = `Contact me in PM for help!`;
  const underattackText = status =>
    `Under attack mode is currently <b>${status}</b>.\n\n` +
    `To change this setting, try this command again followed by on/off.`;
  const underattackAlreadySetText = status =>
    `Under attack mode is already <b>${status}</b>.`;
  ...
  const maintenanceText = status =>
    `Maintenance mode is currently <b>${status}</b>.\n\n` +
    `To change this setting, try this command again followed by on/off.`;
  const maintenanceAlreadySetText = status =>
    `Maintenance mode is already <b>${status}</b>.`;
